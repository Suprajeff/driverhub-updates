...
import com.deliveryhub.uberwatcher.material.internal.DraggableAnchors
...
import com.deliveryhub.uberwatcher.material.internal.systemBarsForVisualComponents
import com.deliveryhub.uberwatcher.material.tokens.MotionSchemeKeyTokens
import com.deliveryhub.uberwatcher.material.tokens.NavigationRailBaselineItemTokens
import com.deliveryhub.uberwatcher.material.tokens.NavigationRailCollapsedTokens
import com.deliveryhub.uberwatcher.material.tokens.NavigationRailColorTokens
import com.deliveryhub.uberwatcher.material.tokens.NavigationRailExpandedTokens
import com.deliveryhub.uberwatcher.material.tokens.NavigationRailHorizontalItemTokens
import com.deliveryhub.uberwatcher.material.tokens.NavigationRailVerticalItemTokens
import com.deliveryhub.uberwatcher.material.tokens.ScrimTokens
...
import com.deliveryhub.uberwatcher.material.internal.BackHandler
...
@OptIn(ExperimentalMaterial3ExpressiveApi::class)
internal fun createDefaultModalWideNavigationRailProperties(): ModalWideNavigationRailProperties {
    return ModalWideNavigationRailProperties() // Just return the default instance
}

@Immutable
@ExperimentalMaterial3ExpressiveApi
class ModalWideNavigationRailProperties(val shouldDismissOnBackPress: Boolean = true)

@ExperimentalMaterial3ExpressiveApi
@Composable
internal fun ModalWideNavigationRailDialog(
    onDismissRequest: () -> Unit,
    properties: ModalWideNavigationRailProperties,
    onPredictiveBack: (Float) -> Unit, // For predictive back progress
    onPredictiveBackCancelled: () -> Unit, // When predictive back is cancelled
    predictiveBackState: RailPredictiveBackState, // The current predictive back state
    content: @Composable () -> Unit,
) {
    // For Android, we typically use a Dialog for modal behavior.
    // The Dialog will handle the 'onDismissRequest' when clicking outside or pressing back.

    // Handle predictive back for Android 14+ (U) and older APIs
    val predictiveBackEnabled = Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE

    if (predictiveBackEnabled) {
        // Android U+ has system-level predictive back APIs.
        // The implementation here is simplified. Real predictive back for dialogs
        // would involve custom dialog implementations and specific listeners.
        // For a direct replacement of `expect`, we'll assume the `onPredictiveBack`
        // and `onPredictiveBackCancelled` are primarily for visual feedback,
        // as the system handles the actual dismissal.

        // Note: A full predictive back integration for a custom dialog would be more complex
        // involving Activity's OnBackPressedCallback, or Compose's BackHandler for manual
        // handling, and potentially intercepting WindowInsets for gesture progress.
        // For this `expect` replacement, we'll use `BackHandler` as a common denominator.

        BackHandler(enabled = properties.shouldDismissOnBackPress) {
            // For predictive back, the system will often animate the dismissal.
            // If the predictive back state is "in progress", we should show a visual.
            // When the gesture completes, onDismissRequest will be called by the system.
            // When cancelled, onPredictiveBackCancelled is called.
            // Here, we just call dismiss as the default.
            onDismissRequest()
        }

        // You might use LaunchedEffect to observe `predictiveBackState` and
        // provide visual feedback, e.g., scaling the dialog content.
//        LaunchedEffect(predictiveBackState) {
//            // This is where you'd typically apply transformations based on predictiveBackState.progress
//            // to the dialog's content for the predictive back animation.
//            // This example simply logs or triggers `onPredictiveBack` if needed.
//            if (predictiveBackState.inProgress) {
//                onPredictiveBack(predictiveBackState.progress)
//            } else if (!predictiveBackState.inProgress && predictiveBackState.progress > 0f) {
//                // If progress was > 0 but now not in progress, it was cancelled.
//                onPredictiveBackCancelled()
//            }
//        }

    } else {
        // For older Android versions, use standard BackHandler
        BackHandler(enabled = properties.shouldDismissOnBackPress) {
            onDismissRequest()
        }
    }
    ...
    Dialog(
        ...
        properties = DialogProperties(
            ...
            dismissOnClickOutside = true, // Common for modal dialogs
            usePlatformDefaultWidth = false // Allow custom width for wide rail
        )
    ) {
        // The content of your ModalWideNavigationRail.
        // This is where you would place your actual NavigationRail composable,
        // possibly inside a Box or Surface that defines its size and background.
        Box(
            modifier = Modifier
                .fillMaxSize() // Or define a specific width/height for the rail
    ...
    Surface(
        ...
        modifier =
            modifier
                ...
                .draggableAnchors(
                    railState.anchoredDraggableState,
